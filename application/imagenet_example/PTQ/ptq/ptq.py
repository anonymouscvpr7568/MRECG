...
def load_calibrate_data(model, train_loader, cali_batchsize):
    cali_data = []
    # origin
...
def main():
    ...
    if not hasattr(config, 'quantize'):
        ...
        cali_data = load_calibrate_data(model, train_loader, cali_batchsize=config.quantize.cali_batchsize)
        ...
        if hasattr(config.quantize, 'reconstruction'):
            model, loss_list = ptq_reconstruction(
                model, cali_data, config.quantize.reconstruction, scheme)
        ...
        print('begin calibration now!')
        cali_data = load_calibrate_data(disable_all(model), train_loader, cali_batchsize=config.quantize.cali_batchsize)
        from mqbench.utils.state import enable_quantization, enable_calibration_woquantization
        # do activation and weight calibration seperately for quick MSE per-channel for weight one
        model.eval()
        enable_calibration_woquantization(model, quantizer_type='act_fake_quant')

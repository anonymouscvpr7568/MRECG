...
def subgraph_reconstruction(subgraph, cached_inps, cached_oups, config, weight):
    global USE_LINK
    global USE_DDP
    ...
    loss_func = LossFunction(subgraph=subgraph, weight=weight, max_count=config.max_count, b_range=config.b_range,
                             warm_up=config.warm_up, weight1=config.weight1)
...
def ptq_reconstruction(model: GraphModule, cali_data: list, config: dict, scheme: list = None, weight: list = [0.1]*18, graph_module_list: list = None):
    r"""
    ...
    for node in nodes:
        ...
        if node.op == "call_module" and isinstance(quant_modules[node], _ADAROUND_SUPPORT_TYPE):
            ...
            if config.pattern == 'layer':
                ...
                layer_node_list = extract_block(node.all_input_nodes, quant_modules, scheme=scheme[count])
                count += 1
            else:
            ...
            rec_loss = subgraph_reconstruction(subgraph, cached_inps, cached_oups, config, weight[weight_count])

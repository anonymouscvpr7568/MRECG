...
class LearnableWeightFakeQuantize(QuantizeBase):
    ...
    def __init__(self, observer, scale=1., zero_point=0., use_grad_scaling=True, **observer_kwargs):
        super(LearnableWeightFakeQuantize, self).__init__(observer, **observer_kwargs)
    ...
    def get_soft_weight(self, x: torch.Tensor):
        if self.ch_axis != -1:
        ...
        return x.clamp(self.quant_min, self.quant_max)
        ...
        if self.fake_quant_enabled[0] == 1:
            ...
            if self.is_per_channel:
                ...
                X = _fake_quantize_learnable_per_tensor_affine_training(
                    ...
                    self.quant_min, self.quant_max, grad_factor, hard_value=self.hard_value)
        return X
...
def _fake_quantize_learnable_per_tensor_affine_training(x, scale, zero_point, quant_min, quant_max, grad_factor, hard_value=False):
    zero_point = (zero_point.round() - zero_point).detach() + zero_point
    # new_shape = [1] * len(x.shape)
    # new_shape[ch_axis] = x.shape[ch_axis]
    scale = grad_scale(scale, grad_factor)
    ...
    if hard_value:
        ...
        x = _rectified_sigmoid(x, quant_min, quant_max)
